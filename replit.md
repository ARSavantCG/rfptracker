...
- **Advanced Systems**:
    ...
    - **RFP Templates System** (November 2025): Comprehensive template management for pre-configured cost item sets. JSON file storage (data/templates.json) with automatic server initialization. Backend module (server/lib/rfp-templates.ts) provides 7 secured API routes: list templates, get template, create, update, duplicate, archive, and preview for import. Admin permissions required for mutating operations (create, update, duplicate, archive). Admin → RFP Templates UI provides full CRUD with searchable table, create/edit modals, and duplicate/archive functionality. Evaluation → Import modal enhanced with dual-tab interface: "From Projects" tab for existing RFP import, "From Templates" tab for template selection. Backend route `/api/templates/:id/for-import` converts template items to evaluation line item format with automatic categorization (Tenant Improvements, Design/Soft Costs, Existing Improvements) based on item tags and type. Frontend integration seamlessly imports template items into evaluation budgets with proper ID generation, toast notifications, and error handling. System ships with 2 seed templates: "Baseline Industrial TI" and "Core & Shell + Utilities". Template items support standard fields (code, label, type, qty, unit_cost, percent, tags, notes) with flexible categorization for reusable cost configurations.
