...
**`canAdvanceToPhase` gate permanently blocked all Step 2 → Step 3 advancement due to legacy no-UI fields.** The `getRequiredFieldsForPhase` function checked for `projectAddress`, `projectSize`, `estimatedValue`, `timelineRequirements` (and `specialRequirements` for later phases) — fields that exist in the schema but have no UI and are NULL for every RFP in the database, including ones already at "publish" phase. These fields became gated blockers when duplicate workflow-phase handlers were merged earlier this session-pair, activating previously-dead code. Fix: emptied `getRequiredFieldsForPhase` to return `[]` for all phases (schema columns preserved — no migration). Real required fields (`contractorDueDate`, `architectDueDate`, etc. from `validateRfpForProgression`) remain gated as before. Files changed: `server/validation.ts`.
...
**Pattern: merging duplicate route handlers can activate dormant business rules.** When the dead handler contains logic the live handler lacks, that logic was previously unreachable — merging it in is functionally adding a new check, not preserving an existing one. Always verify the data the dormant rule depends on actually exists in production records before activating. This session-pair has now had TWO bugs caused by exactly this pattern: (1) the `canAdvanceToPhase` gate blocking all Step 2 advancement because it checked legacy no-UI fields, and (2) a prior publish-email side effect. Rule: when merging dead handlers, audit every business-rule condition for data availability against live records before shipping.
