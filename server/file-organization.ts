import { WORKFLOW_PHASES } from '@shared/schema';
import fs from "fs";
import path from "path";

/**
 * Folder per workflow step, DERIVED from WORKFLOW_PHASES.
 *
 * This used to be written out by hand and had drifted: invitation-to-bid and
 * bid-collection shared one folder, so evaluation filed as Step_4 and publish as
 * Step_5 while the UI called them 5 and 6. Deriving it means the folder number
 * and the step number cannot disagree again.
 */
export const WORKFLOW_STEP_MAPPING: Record<string, string> = Object.fromEntries(
  WORKFLOW_PHASES.map(p => [p.key, p.folder])
);
...
export const WORKFLOW_STEP_FOLDERS = [
  ...WORKFLOW_PHASES.map(p => p.folder),
  // Sub-folders under Evaluation, kept explicit because they are not phases.
